...
### 1) ATT(앱 추적 투명성) 문구 추가 — 필수

`Info.plist` 에 "Privacy - Tracking Usage Description"(`NSUserTrackingUsageDescription`) 을
추가합니다. 여기에 작성된 문구는 앱 추적 동의 팝업에 노출됩니다. 광고 로드 직전에
플러그인이 ATT 권한을 요청하며(앱 생애 최초 1회), 광고 ID(IDFA) 확보 시 광고 fill 이 향상됩니다.

```xml
<key>NSUserTrackingUsageDescription</key>
<string>사용자에게 적합한 광고를 제공하고 참여 여부를 확인하기 위하여 광고 ID를 수집합니다. 광고 ID는 서비스 제공을 위한 필수 항목으로, 추적 허용을 해주셔야 사용이 가능합니다.</string>
```

### 2) pub id 등록

앱 등록 후 발급받은 pub id 를 `Info.plist` 에 `tnk_pub_id` 항목으로 추가합니다.
(테스트 단계에서는 없이도 테스트 광고 출력이 가능합니다.)

```xml
<key>tnk_pub_id</key>
<string>YOUR-INVENTORY-ID-HERE</string>
```
...
(iOS 는 최초 호출 시 ATT 팝업이 먼저 노출됩니다.)
...
앱 등록 후 pub id 를 발급받아 위 [iOS 프로젝트 설정](#2-pub-id-등록)처럼 `Info.plist` 에
`tnk_pub_id` 로 추가합니다. 실제 ID 를 등록하면 테스트 코드에서는 더 이상 광고가 나타나지
않으며, Tnk Publish Site 에서 광고 유형에 맞춰 Placement 를 등록하고 그 이름을 `placementId`
로 사용해야 실제 광고가 표시됩니다.
